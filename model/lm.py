...
class LanguageModel(nn.Module):
    def __init__(self, vocab, hp):
        ...
        if self.hp.use_rec:
            self.encoder_rec = Encoder(input_dim=self.input_dim,
                                       ...
                                       emb_dim=self.hp.emb_dim,  # Ensuring the embedding dimension is consistent
    ...
    def classify(self, src_senti, cls_hidden, rec_hidden, alpha):
        ...
        # Concatenate target domain doc representations
        if self.num_tgr_docs_domains == 1:
        ...
        # Gradient Reversal Layer (if applicable)
        if self.hp.use_grl:
...
def sample_target_docs(domain, hidden, batch_size, pool="same", concat=False):
    ...
    for idx in range(batch_size):
        ...
        if h.size(0) == 0:
            # If no target docs available, use the current document's hidden
